...
import (
	"encoding/json" //json.RawMessage没法替代
	jsoniter "github.com/json-iterator/go"
	"net/http"

	"github.com/maclon-lee/golanglib/lib/rpc"
)
...
func newCodecRequest(r *http.Request, encoder rpc.Encoder, errorMapper func(error) error) rpc.CodecRequest {
	...
	var gjson = jsoniter.ConfigCompatibleWithStandardLibrary
	req := new(serverRequest)
	err := gjson.NewDecoder(r.Body).Decode(req)
...
func (c *CodecRequest) ReadRequest(args interface{}) error {
	...
	if c.err == nil && c.request.Params != nil {
		...
		if err := gjson.Unmarshal(*c.request.Params, args); err != nil {
			// Clearly JSON params is not a structured object,
			// fallback and attempt an unmarshal with JSON params as
			// array value and RPC params is struct. Unmarshal into
			// array containing the request struct.
			params := [1]interface{}{args}
			if err = gjson.Unmarshal(*c.request.Params, &params); err != nil {
				c.err = &Error{
					Code:    E_INVALID_REQ,
					Message: err.Error(),
...
func (c *CodecRequest) writeServerResponse(w http.ResponseWriter, res *serverResponse) {
	...
	if c.request.Id != nil || isParseErrorResponse(res) {
		...
		encoder := gjson.NewEncoder(c.encoder.Encode(w))
